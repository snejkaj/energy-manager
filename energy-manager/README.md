...
All configuration is optional. Missing Tibber, Tesla, weather, solar, database, or charger settings produce setup guidance in the UI instead of blocking startup.
...
Use the Home Assistant add-on configuration screen for real tokens. For local development, keep secrets in `.env` or `options.json`; both are ignored by git. Startup logs only show whether a token is configured, never the token itself.
